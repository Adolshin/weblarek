...
Интерфейс IHeader описывает тип объекта, который принимает метод render родительского класса, передается дженериком
```
interface IHeader {
  counter: number;
}
...
Конструктор:  
`constructor(container: HTMLElement)` - принимает ссылку на DOM элемент для передачи в конструктор родительского класса
...
Вспомогательный интерфейс:
Интерфейс ICatalog описывает тип объекта, который принимает метод render родительского класса, передается дженериком
```
interface ICatalog {
  content: HTMLElement[];
}
